...
CHAR_RANGE_LIST = [KSX_1001, ADOBE_KR9]
CHAR_RANGE_DICT= [{string : string for string in KSX_1001}, {string : string for string in ADOBE_KR9}]
...
def compose(chosung, jungsung, jongsung=u''):
    ...
    if jongsung is None or jongsung is u'·' : jongsung = u''
    ...
    composed = unichr(0xAC00 + chosung_index * NUM_JUNG * NUM_JONG + jungsung_index * NUM_JONG + jongsung_index)
    try:
        if CHAR_RANGE == 2 :
            return composed
        else :
...
def composeAllCases(chosung = [u''], jungsung = [u''], jongsung = [u'']):
    ...
    if is_cho(chosung)==[] and is_jung(jungsung)==[] and is_jong(jongsung)==[] :
        print('자소를 선택해주세요.')
        return CHAR_RANGE_LIST[CHAR_RANGE]
...
""".format(idx = idx))
        ...
        self.comboBox_charRange.addItem(u'KS X 1001 - 2350자', 0)
        self.comboBox_charRange.addItem(u'ADOBE-KR-9 - 2780자', 1)
        ...
        self.comboBox_sortOrder.addItem(u'중성 모임꼴 순', 2)
        self.comboBox_sortOrder.addItem(u'종성 순', 3)
        ...
        self.layout_main.addWidget(self.label_strLen,           7,0,1,6)
        ...
        self.gridLayout_controls.addWidget(self.btn_copy,               2,0,1,1)
        self.gridLayout_controls.addWidget(self.btn_editOnCurTab,       2,1,1,1)
        self.gridLayout_controls.addWidget(self.btn_editOnNewTab,       2,2,1,1)
        self.gridLayout_controls.addWidget(self.btn_compose,            3,0,1,3)
...
QGroupBox > QPushButton{
    ...
    def refreshText(self):
        self.composedChars = composeAllCases(self.selected_cho, self.selected_jung, self.selected_jong)
        self.textEdit_output.setText(''.join(self.composedChars))
        self.label_strLen.setText('조합된 글자: {}자'.format(len(self.textEdit_output.toPlainText())))
